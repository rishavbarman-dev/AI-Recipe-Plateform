...
export async function getOrGenerateRecipe(formData) {
  try {
    ...
    // Step 2: Recipe does't exist, generate with Gemini
...
export async function saveRecipeToCollection(formData) {
  try {
    ...
    const existingResponse = await fetch(
      `${STRAPI_URL}/api/saved-recipes?filters[user][id][$eq]=${user.id}$filters
...
export async function removeRecipeFromCollection(formData) {
  try {
    ...
    if (!searchData.data || searchData.data.length === 0) {
      return {
        success: true,
        message: "Recipe was not in you collection",
      };
    }
...
export async function getRecipesByPantryIngredients() {
  try {
    ...
    if (!pantryResponse.ok) {
      console.error(
        "Failed to fetch pantry items. Status:",
        pantryResponse.status,
        "Status Text:",
        pantryResponse.statusText,
      );
